...
PresenterFactory
implements
IPresenterFactory{public$caseSensitive=FALSE;public$mapping=array(NULL=>'\*Module\*Presenter','Nette'=>'NetteModule\*\*Presenter');private$baseDir;private$cache=array();private$container;function
...
formatPresenterClass($presenter){$parts=explode(':',$presenter);$mapping=explode('\\*',isset($parts[1],$this->mapping[$parts[0]])?$this->mapping[array_shift($parts)]:$this->mapping[NULL]);$class=$mapping[0];while($part=array_shift($parts)){$class.=($class?'\\':'').$part.$mapping[$parts?1:2];}return$class;}function
...
function
setContext(Nette\DI\Container$context){$this->context=$context;if($this->invalidLinkMode===NULL){$this->invalidLinkMode=empty($context->parameters['debugMode'])?self::INVALID_LINK_SILENT:self::INVALID_LINK_WARNING;}}final
function
...
class
Framework{const
NAME='Nette Framework',VERSION='2.1-dev',REVISION='b8c66ea released on 2012-04-28';public
...
setItems(array$items,$useKeys=TRUE){$this->items=$this->prompt===TRUE?array(''=>$this->items[''])+$items:$items;$this->allowed=$this->prompt===TRUE?array(''=>''):array();$this->useKeys=(bool)$useKeys;foreach($items
...
function
processHtmlAttribute(Token$token){if(Strings::startsWith($token->name,Parser::N_PREFIX)){$this->htmlNode->macroAttrs[substr($token->name,strlen(Parser::N_PREFIX))]=$token->value;}else{$this->htmlNode->attrs[$token->name]=TRUE;$this->output.=$token->text;if($token->value){$context=NULL;if(strncasecmp($token->name,'on',2)===0){$context=self::CONTENT_JS;}elseif($token->name==='style'){$context=self::CONTENT_CSS;}$this->setContext($token->value,$context);}}}function
